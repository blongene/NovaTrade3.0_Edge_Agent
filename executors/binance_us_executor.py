...
# Binance.US MARKET executor with BUY quote-guard, SELL clamps, lot-size handling, and provenance fields.
...
class BinanceUS:
    def __init__(self):
        self.key  = (os.getenv("BINANCEUS_API_KEY") or "").strip()
        self.sec  = (os.getenv("BINANCEUS_API_SECRET") or "").strip()
        self.base = BUS_BASE
        self.sess = requests.Session()
        self.sess.headers.update({"User-Agent":"NovaTrade-Edge-BinanceUS/1.4","X-MBX-APIKEY": self.key})
    ...
    def ticker_price(self, symbol:str) -> float:
        ...
        try: return float((r.json() or {}).get("price") or 0.0)
...
def _symbol_norm(venue_symbol: str) -> str:
    # Accept BTC/USDT, BTCUSDT, BTC-USDT → BTCUSDT. If accidentally /USDC, rewrite to /USDT.
    s = (venue_symbol or "").replace("-", "").replace("/", "").upper()
    if s.endswith("USDC"): s = s[:-4] + "USDT"
    return s
...
def execute_market_order(*, venue_symbol: str, side: str,
    ...
    requested = (venue_symbol or "").upper()
    symbol    = _symbol_norm(venue_symbol)  # resolved to BUS format, e.g., BTCUSDT
    ...
    # ---- BUY path: quote balance guard + min notional
    if side_uc == "BUY":
    ...
    # ---- SELL path: clamp to free + floor to step + minQty
    else:
        acct = bus.account()
        free_btc = 0.0
        for b in acct.get("balances", []):
    ...
    # ---- handle response
    if resp is None:
        return {"status":"error","message":data.get("error","bad params"),"fills":[],
                "venue":"BINANCEUS","symbol":symbol,
                "requested_symbol":requested,"resolved_symbol":symbol,"side":side_uc}
    ...
    # Post-trade balances snapshot (USDT, BTC)
    post = {"USDT": None, "BTC": None}
    try:
        acct2 = bus.account()
    ...
    return {"status":"ok","txid": order_id or f"BUS-NOORD-{int(time.time()*1000)}","fills":fills,
            "venue":"BINANCEUS","symbol":symbol,
            "requested_symbol":requested,"resolved_symbol":symbol,
