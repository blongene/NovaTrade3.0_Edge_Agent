...
# executors/kraken_executor.py  (BUY guard + balances)
...
def _balance() -> Dict[str, float]:
    # returns Kraken symbols: XBT, USDT, USDC, etc.
    s = _sign("/0/private/Balance", {})
    r = requests.post(f"{BASE}/0/private/Balance", data=s["qs"], headers=s["hdr"], timeout=15)
    j = r.json()
    if j.get("error"): raise RuntimeError(",".join(j["error"]))
    out = {}
...
def execute_market_order(*, venue_symbol: str, side: str,
    ...
    pair = _sym(requested)  # resolved for Kraken (XBTUSDT)
    side_uc = (side or "").upper()

    if edge_hold:
        return {"status":"held","message":"EDGE_HOLD enabled","fills":[],
                "venue":"KRAKEN","symbol":pair,"requested_symbol":requested,"resolved_symbol":pair,"side":side_uc}
    ...
    if edge_mode != "live":
        ...
        return {"status":"ok","txid":f"SIM-KR-{int(time.time()*1000)}","fills":[{"qty":qty,"price":px}],
                "venue":"KRAKEN","symbol":pair,"requested_symbol":requested,"resolved_symbol":pair,
                "side":side_uc,"executed_qty":qty,"avg_price":px,"message":"kraken dryrun simulated fill"}
    ...
    # Quote-balance precheck for BUY
    if side_uc == "BUY":
        ...
        if q_spend <= 0:
            return {"status":"error","message":"BUY requires amount_quote > 0","fills":[],
                    "venue":"KRAKEN","symbol":pair,"requested_symbol":requested,"resolved_symbol":pair,"side":side_uc}
        free = 0.0
        try: free = float(_balance().get("USDT", 0.0))
        except Exception: free = 0.0
        if q_spend > free:
            return {"status":"error","message":f"insufficient USDT: have {free:.2f}, need {q_spend:.2f}","fills":[],
                    "venue":"KRAKEN","symbol":pair,"requested_symbol":requested,"resolved_symbol":pair,"side":side_uc}
        # compute rough qty (venue computes final)
        px = 0.0
        try: px = float(_ticker_price(pair))
        ...
        # SELL clamp vs free XBT
        bals = {}
        try: bals = _balance()
        except Exception: pass
        free_xbt = float(bals.get("XBT", 0.0))
    ...
    # Post-trade balances snapshot (USDT/XBT)
    post = {}
    try:
        b = _balance()
    ...
    return {"status":"ok","txid": txid or f"KR-NOORD-{int(time.time()*1000)}","fills":[],
            "venue":"KRAKEN","symbol":pair,"requested_symbol":requested,"resolved_symbol":pair,"side":side_uc,
            "post_balances": post, "message":"kraken live order accepted" if txid else "kraken response parsed"}
